...
- Gemma 4 E2B & E4B added to the model catalog, each with an F16 mmproj vision-projector companion download and its own prompt template ([#22](https://github.com/LucknowAI/loxi_llm/issues/22))
- Chat generation is now grounded in an attached image for vision-capable models: the native image marker is embedded into the current turn's prompt and the image is passed through to `generate()`, for both plain chat and the agent tool-calling loop ([#24](https://github.com/LucknowAI/loxi_llm/issues/24))
- RAM guard for vision models: the load-confirmation dialog now breaks out the vision component's memory footprint from the base model's, and a large-model load failure gets a friendlier "not enough free memory" message instead of the raw native exception ([#25](https://github.com/LucknowAI/loxi_llm/issues/25))
