...
- Android: Gemma 4 GGUF models failed to load (`unknown model architecture: 'gemma4'`) because the vendored `llama.cpp` submodule predated Gemma 4 support. Bumped the submodule to upstream `master`, which adds the architecture ([#52](https://github.com/LucknowAI/loxi_llm/issues/52))
