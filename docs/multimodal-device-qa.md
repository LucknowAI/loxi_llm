...
- If a `send()` fails *after* the message was already persisted (rare — e.g. a native
  hiccup calling `mediaMarker()`), the composer may restore the just-sent image as a
  pending attachment even though a message with that same image already exists in the
  conversation. `#59` tracks this. Don't file a fresh issue for it — just don't be
  surprised by a duplicate-looking attachment.
