...
**Fixed during this checklist's own first run** (kept here for context, not something
to re-test specially): the composer wasn't clearing its text field after a vision send
...
- [ ] A device with **<6 GB RAM** available (or use Settings/adb to simulate — see
...
- [ ] A device with **≥6 GB RAM**, to confirm the warning correctly does *not* fire
      there.
- [ ] Wi-Fi, to download Gemma 4 E2B (~3.1 GB) or E4B (~5.0 GB) plus their mmproj files
      (~1 GB each).

## 1. Catalog, download, and load

- [ ] Models tab shows "Gemma 4 E2B IT (Vision)" and "Gemma 4 E4B IT (Vision)" in the
...
- [ ] Downloading one fetches **both** the base model and its mmproj file, with combined
      progress shown as a single percentage (not two separate downloads to watch).
- [ ] Tapping Load shows the full-screen loading overlay (pulsing animation + the
...
- [ ] Load completes successfully; a SnackBar confirms "`<model>` loaded — ready to chat".
- [ ] Retry-after-failure: if a load ever fails (e.g. force-quit mid-load once, then
...
- [ ] On the **<6 GB RAM** device: tapping Load on Gemma 4 E2B/E4B shows the "High
...
- [ ] Tapping "Load Anyway" proceeds; tapping "Cancel" leaves the model `downloaded`
      (not stuck in a weird state).
- [ ] On the **≥6 GB RAM** device: loading the same model shows **no** warning dialog at
      all.
- [ ] A text-only model under ~2 GB (Gemma 3 270M) never shows the warning on any
      device.

## 3. Image-grounded generation (plain chat)

- [ ] With a vision model loaded, the image-attach button appears in the chat composer.
      ...
      one).
- [ ] Attach a photo, send a text prompt referencing it (e.g. "what's in this image?").
...
- [ ] Send a **follow-up text-only message** in the same conversation (no new image).
...
- [ ] Start a **new conversation**, send a **text-only** message with the same vision
...
- [ ] Enable tool-calling (agent mode) for a conversation on an agent-capable vision
      ...
      just the first.
- [ ] Expect this to be **slower** than a single-turn vision response — each agent
...
- [ ] Attach an image, then delete the underlying file from device storage (e.g. via a
...
- [ ] Attempt to tap Load on a second model while a load is already in flight (fast
...
- [ ] With TalkBack (or VoiceOver) on, confirm you cannot navigate to and activate a
...
- [ ] Send a normal (successful) message with an image attached to a vision model.
      Confirm the composer's text field and image thumbnail both clear immediately
      after tapping send, before generation even starts — not just eventually, and
      not only for text-only sends. (Note: for an agent-mode turn, `send()` doesn't
...
- [ ] Text-only chat, RAG, agent mode, conversation export, TTS read-aloud, and document
