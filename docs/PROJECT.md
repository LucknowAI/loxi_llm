...
`ConversationScreen` now lets the user attach an image to a message: an image button in the composer (gated on `isMultimodalModel(loadedModel)`) opens a bottom sheet to take a photo or pick from the gallery via `image_picker`, copies the result into `<app-documents>/images/` via `FileStorageService.getImagePath()` (so it outlives the picker's temp file), and shows it as a small removable thumbnail above the text field. `ChatNotifier.send()` takes an optional `imagePath` and persists it on the user `Message`; `MessageBubble` renders it above the message text. None of this feeds the prompt yet — the image is stored and displayed, not (yet) seen by the model.
